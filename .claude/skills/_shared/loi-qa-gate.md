...
| **R-24** | **fail** | **Cover + body** | **Party duplication: if the cover carries both party blocks (labels `Between:` / `And:` or `By and between:` / `And:`), the body must not contain a heading matching `^\s*THE\s+UNDERSIGNED\b` (case-insensitive). The cover is already the declaration of parties; repeating them in a body UNDERSIGNED block is a semantic duplicate and creates a three-way maintenance burden (cover, UNDERSIGNED block, signature block) instead of two.** | **Pipeline B (`rebrand.py`, M3): strip the UNDERSIGNED block automatically. Pipeline A (generated from YAML): no auto-fix — template must not emit it. Enforced at build-time by `audit_agreement()` in `document-factory/audit_profiles.py`.** |
