...
- **Site Sourcing:** the Site LOI engine (`sites/loi/generate_site_loi.py`) and Site HoT engine (`sites/hot/generate_site_hot.py`) are **LIVE** as of v4.0-rc1; refined to one-source-of-truth architecture in v4.0-rc3 (rc3.1 — chassis-owned `normalise_placeholder` + `PARSER_MAP`; rc3.2 — bilingual cover, schedule-table primitive, auto-margin; rc3.3 — `site_clause_library` data-driven §1; rc3.4 — fontTools-measured `validate_cell_overflow` + advisory visual-regression CI). Both produce bilingual EN/NL `.docx` artefacts via `document-factory/bilingual_body.py` + `signature_block.py`. LFS template binaries resolved (real Word files in `sites/hot/templates/`). End-to-end Van Gog LOI smoke produces a bilingual `.docx` with zero format-validator issues; cross-doc gate surfaces expected `DataAcc-1` + `Esc-2` verdicts per registry `escalation_rules`. CI gates `sites/` on each PR (`legal-assistant-tests.yml`) — scaffolding-marker gate blocks rc-level `TODO(Phase ...)` from reaching production. See `sites/_shared/sal_runbook.md` for operator workflow.
